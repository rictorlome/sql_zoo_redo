...
def richer_than_england
  ...
  # 'United Kingdom'.
  execute(<<-SQL)
  SELECT name FROM countries WHERE (gdp/population) > (
...
def neighbors_of_certain_b_countries
  ...
  SELECT name, continent FROM countries WHERE continent IN (
    SELECT continent FROM countries WHERE name = 'Belize' OR name = 'Belgium'
  )
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
  SELECT name, continent, population FROM countries WHERE continent NOT IN (
